    ...
    const { active, over } = event;
      ...
      if (activeType === "chapter") {
        ...
        if (oldIndex === -1 || newIndex === -1) {
          toast.error("Không thể tìm thấy chương cũ/chỉ mục mới để sắp xếp lại");
          return;
        }
        ...
        if(courseId) {
          ...
          toast.promise(reorderPromise(), {
            ...
            error: () => {
              ...
              return "Không thể sắp xếp lại các chương";
            }
          });
        }
        return;
      ...
      if(activeType === "lesson" && overType === "lesson") {
        ...
        if(!chapterId || chapterId !== overChapterId) {
          toast.error("Bài học di chuyển giữa các chương khác nhau hoặc ID chương không hợp lệ không được phép.")
          return;
        }
        ...
        if(oldLessonIndex == -1 || newLessonIndex === -1) {
          toast.error("Không thể tìm thấy bài học để ghi");
          return;
        }
        ...
        if(courseId) {
          ...
          toast.promise(reorderLessonsPromise(), {
            loading: 'Sắp xếp lại các bài học... ',
            success: (result) => {
              if(result.status === "success") return result.message;
              throw new Error(result.message);
            },
            error: () => {
              setItems(previousItems);
              return 'Không thể sắp xếp lại các bài học';
            }
          });
        }
        return;
